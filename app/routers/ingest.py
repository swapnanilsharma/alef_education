...
async def ingest_pdf(file: UploadFile = File(...)) -> IngestResponse:
    ...
    try:
        ...
        logger.exception(
            "PDF extraction failed | request_id=%s | filename=%s", request_id, filename
        )
        raise HTTPException(
    ...
    try:
        (
            ...
            chunk_count,
            model_id,
        ) = build_embeddings_from_extracted_pdf(
        ...
        logger.warning(
            "Embedding build rejected | request_id=%s | reason=%s", request_id, exc
        )
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:  # noqa: BLE001 - convert Bedrock/runtime issues into API error responses
        logger.exception(
            "Embedding build failed | request_id=%s | source_json=%s",
            request_id,
            output_path,
        )
        raise HTTPException(
            status_code=502, detail=f"Failed to build embeddings: {exc}"
        ) from exc
