...
async def ask_question(request: AskRequest) -> AskResponse:
    ...
    try:
        ...
        logger.warning("Ask request missing vectorstore artifacts | request_id=%s | reason=%s", request_id, exc)
        raise HTTPException(status_code=404, detail=str(exc)) from exc
    except ValueError as exc:
        logger.warning("Ask request rejected | request_id=%s | reason=%s", request_id, exc)
